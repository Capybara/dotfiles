...
set -o vi
if [[ -n "$SSH_CLIENT" ]] ; then  curl --silent https://prowl.weks.net/publicapi/add -F apikey="1b7952dcf26429c8496dba07730f2977523e3868" -F priority=2 -F application="SSH-Notify" -F event="SSH Connection" -F description="$SSH_CLIENT""  has made a ssh connection to cerdo" >/dev/null 2>&1; fi
